...
#                 learning_rate = 0.001
            ...
            val_rmse_score = self.calculate_rmse(prediction, self.val)
    ...
    def prediction_conversion(self, current_ovr_rooms=None):
        """
        filtering current OVR rooms and sorting the predicted rating for each user

        """ 
        
        self.prediction_remapping()
        
        if current_ovr_rooms:
            self.predicted_df = self.predicted_df[self.predicted_df['item_id'].isin(current_ovr_rooms)]
    ...
    def actual_pred_df(self,val_df):
        ...
        ---------- Input ---------------
        dataframe : user_id - sorted order of recommended artwork(only for current OVR) -rating
