...
create the user-item(room)-rating matrix.
...
class Useraffinityroom():
    ...
    def useraffinity_preprocessing(self,tracking_df):     
        ...
        print('-----------Useraffinityroom preprocessing started-----------')
        ...
        print('Dataset Shape after removing duplicates: ',df_fil.shape)
    ...
    def dwelltime_visits_calculation(self,preprocessed_df):
        ...
        """
        deriving dwelling time for each room 
        
        ---------- Input ---------------
        dataframe: preprocessed dataframe
        
        ---------- output ---------------
        dataframe: dataframe with dwelltime and no of visits for each room
        
        """   
        
        
        #deriving dwelltime for each room by finding the time difference between two performed action
        ...
        #filterout the dwelltime >3 and (threshold)
        filtered_dataframe=preprocessed_df[(preprocessed_df['dwell']>3)]
        
        #filterout the performed action ='ENTER_ROOM(1)'
        filtered_dataframe=filtered_dataframe[filtered_dataframe['PerformedAction'].isin([1])]
        ...
        #aggregation of visits and room
    ...
    def Merge_collections_with_tracking(self,collection_df,tracking_aggregated_df):
        ...
        """
        Merge tracking_data with collection_data to find whether the user has any collected room or not
        ...
        #entity_type filtration on artworks with rooms---4c       
        df_final_fil=df_final_fil[df_final_fil['ENTITY_TYPE'].isin(['4c'])]
    ...
    def log_calculation(self,collection_tracking_df):
        ...
        """
        deriving the log value for dwelltime - Non Linear transformation
